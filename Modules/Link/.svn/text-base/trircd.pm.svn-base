...
sub link_init
{
        ...
        main::provides("server","trircd-server");
}
...
sub gline
{
	...
	&rawirc(":$botnick SGLINE * $delta $ident $host :AKILL active: $reason");
	$KILLED++;
}
...
sub connect {
	...
	print ("Logging in...\n");
	&rawirc("PASS $password :TS7");
	&rawirc("CAPAB NICKIP TRIRCD5 SERVICE");
	&rawirc("MYID !$numeric");
	&rawirc("SERVER $servername 1 :$serverdesc");

	print ("Introducing pseudoclient: $botnick...\n");
	#NICK Defender 1 1088041686 +o Defender Defender.AccessIRC.Net Defender.AccessIRC.Net Defender.AccessIRC.Net 0 0 :Defender of the network
	my $now = time;
	&rawirc("NICK $botnick 1 $now +aiorw $botnick $domain $servername $servername 0 0 :$botname");
	&rawirc("SVSMODE $botnick $now -R");

	print ("Joining channel...$mychan\n");
...
#SERVER <servername> <hopcount> :<server description>
		...
		if ($buffer =~ /^SERVER\s(.+?)\s(.+?)\s:(.+?)/)
		{
			$NETJOIN = 1;
			$njservername = $1;
			print "uplink ($njservername) is synching...\n";
			$njtime = time+80;
		}
